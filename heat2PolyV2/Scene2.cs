...
namespace fluidedotnet
{
  ...
  public class Scene
  {
    ...
    public int runcount;
    public bool drawinfo = false; // разрешает вывод информации о элементах
    public bool drawnbinfo = false;
    public bool drawfacepoints = true;
    public bool drawelementid = false; // выводит id элементов
    //  public bool   byonestep;
    // public bool   readytostep;    

//    public bool drawgrid = true;
//    public bool fillelement = false;

    public bool drawgrid = false; // рисует mesh
    public bool fillelement = true; // закраска элементов в соответствии со значением
  
    public string filename;
    public bool drawbndfaces = false; //рисует граничные грани 
    ...
    SolidBrush textBrushRed = new SolidBrush(Color.Red);
    ...
    public Scene()
    {
      ...
      drawchild = true;
      zoom = 1;
      localzoom = 45; // основной параметр маштаба
      ValueColorCoeff = 1.0;
//      colormap = Colormap.Grayscale;
      colormap = Colormap.HSV; // цветова€ схема
//      colormap = Colormap.Cool;
      sleeptimems = 0; // можно задать задержку дл€ покадровой анимации шагов по времени
      runcount = 40;// дл€ покадровой анимации задать=0
      //dh = 13.0;
      //dh = 30.0;
      //dh = 45.0;
      //dh = 60.0;
...
//          else
    ...
    public void FillPath(Graphics gdi, Brush b, Vector2[] vertex)
    {
        double totalzoom = zoom * localzoom;
       Point[] points=new Point[vertex.Length];

        for(int n=0;n<vertex.Length;n++)
        {
            points[n]=new Point((int)(startx +vertex[n].x * totalzoom),(int)(starty -vertex[n].y * totalzoom));
        }
        GraphicsPath path = new GraphicsPath();
...
*/
    ...
    public void FillElementNodes(Graphics gdi, Element elem)
    {
        ...
        int n = 0;
        for (n = 0; n < elem.vertex.Length; n++)
        {
    ...
    public Color ValueToHSV(double value)
    {
        int r1, g1, b1;
        HsvToRgb(255 - value, 1, 1, out r1, out g1, out b1);
        return Color.FromArgb(r1, g1, b1);
    }
    public Color ValueToGrayScale(double value)
    {
        if (value > 255)
            value = 255;
        return Color.FromArgb((int)value, (int)value, (int)value);
    }
    public Color ValueToCool(double value)
    {
        if (value > 255)
            value = 255;
        return Color.FromArgb((int)value, 44,185);
    }
    public Color ValueToColor(double value)
    {
        if(colormap==Colormap.HSV)
            return ValueToHSV(value);
        else  if (colormap == Colormap.Grayscale)
                return ValueToGrayScale(value);
        else
          return ValueToCool(value);
    }
...
//        p[0] = f.pointid1;
//        p[1] = f.pointid2;
//        p[2] = elem.faces[0].pointid2;

/*
        for (n = 0; n < elem.faces.Length; n++)
        {
            Face face = elem.faces[n];
        }
 */
  //      p[0] = elem.faces[0];
  //      p[1] = -1;
  //      p[2] = -1;
        ...
        for (n = 0; n < elem.faces.Length; n++)
        {
            if (elem.faces[n].owner != elem)
                continue;

            if (drawbndfaces && elem.faces[n].isboundary)
            {
                cx1 = (int)(elem.faces[n].vertex[0].x * totalzoom);
                cy1 = (int)(elem.faces[n].vertex[0].y * totalzoom);
                cx2 = (int)(elem.faces[n].vertex[1].x * totalzoom);
                cy2 = (int)(elem.faces[n].vertex[1].y * totalzoom);
                Pen p = new Pen(ValueToColor(elem.faces[n].bnddomain * 50));
                p.Width = 4;
                DrawLine(gdi, p, cx1, cy1, cx2, cy2);
            }
            if (drawnbinfo)
            {
                cx1 = (int)(elem.faces[n].centroid.x * totalzoom);
                cy1 = (int)(elem.faces[n].centroid.y * totalzoom);
                s = "( " + elem.faces[n].owner.id.ToString();
                if(elem.faces[n].nbelem!=null)
                  s+=" - " + elem.faces[n].nbelem.id.ToString() + " )";
                else
                    s += " )";
                DrawString(gdi, s, font, textBrushRed, cx1 - 17, cy1 - 15);
            ...
            if (drawfacepoints)
            {
                cx1 = (int)(elem.faces[n].centroid.x * totalzoom);
                cy1 = (int)(elem.faces[n].centroid.y * totalzoom);
                s = "(" + elem.faces[n].pointid1.ToString() + "," + elem.faces[n].pointid2.ToString() + ")";
                DrawString(gdi, s, font, textBrushGreen, cx1 - 17, cy1);
            }
        }
    ...
    public void RenderParent(Graphics gdi, int x, double tmax)
    {
        ...
        if(t<0)
            t=0;
//        double t = (elem.faces[0].bndu+elem.faces[1].bndu+elem.faces[2].bndu);
        double tc = t * ValueColorCoeff;// -240 * 3 - 80;
        Brush b = new SolidBrush(ValueToColor((int)tc));
        FillPath(gdi, b, elem.vertex);
