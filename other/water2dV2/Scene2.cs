...
namespace fluidedotnet
{
  ...
  public class Scene
  {
    ...
    public bool drawpressure ;
    public bool drawvectorhat;
    public bool drawfast;
    public bool drawparticles ;
    public bool drawId ;
    public bool drawchild ;
...
//          else
    ...
    public void RenderParentGrid(Graphics gdi, int x, int y)
    {
        ...
        if (fluid.IsBoundary(x, y))
        {
            FillRectangle(gdi, Brushes.LightGreen, cellx1, celly1, cellx2, celly2);
            DrawRectangle(gdi, Pens.Gray, cellx1, celly1, cellx2, celly2);
        }
        else
        {
    ...
    public void RenderParent(Graphics gdi, int x, int y, int areacoeff, double tmax)
    {
        ...
        if (drawvelocity)
        {
            double u;
            double v;
            u = fluid.GetU(x, y) * ValueColorCoeff;
            v = fluid.GetV(x, y) * ValueColorCoeff;

            Vector2 vv = new Vector2(u,v);

            double t = vv.Length();
            double tc = Math.Abs(t) * ValueColorCoeff;
        ...
        else if (drawpressure)
        {
            double t = fluid.GetP(x, y) * ValueColorCoeff;
            if (t < 0)
                t = 0;
            double tc = t;
//            double tc = Math.Abs(t);
        ...
        else
        {
            ...
            int x1 = (int)(cellx1 + cellx2 / 2);
            int y1 = (int)(celly1 + celly2 / 2);
            //        int x2 = (int)(x1 + u);
            //        int y2 = (int)(y1 + v);
            int x2 = (int)(u);
            int y2 = (int)(v);

            //        DrawLine(gdi,Pens.Red, x1, y1, x2, y2);
            DrawVectorReal(gdi, Pens.Red, x1, y1, x2, y2, 15);
        }
